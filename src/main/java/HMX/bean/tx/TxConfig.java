 ...
 *  3.给方法上标注上@Transactional注解表明这是一个事务方法
 *  4.@EnableTransactionManagement //开启事务管理功能
 *  5.配置事务管理器在容器中
 *
 *
 *  原理:
 * 1).@EnableTransactionnaManagement
 ...
 *        利用后置处理器在对象创建以后,包装对象,返回一个代理对象(增强器)
 *        代理对象执行方法利用拦截器链进行调用
 * 3).ProxyTransactionManagementConfiguration做了什么
 *     1.给容器中注册事务增强器:
 *         1).增强器要用事务注解的信息,AnnotationTransactionAttributeSource解析事务注解
 *         2).事务拦截器:
 ...
 *                 事务拦截器:
 *                   1).先获取事务的相关属性
 *                   2).在获取PlatformTransactionManager,如果事先没有添加指定TransactionManager
